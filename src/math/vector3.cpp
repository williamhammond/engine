...
engine::Vector3::Vector3(float x_in, float y_in, float z_in) {
  x = x_in;
  y = y_in;
  z = z_in;
}

float& engine::Vector3::operator[](int i) { return ((&x)[i]); }

const float& engine::Vector3::operator[](int i) const { return ((&x)[i]); }

[[nodiscard]] std::string engine::Vector3::ToString() const {
